...
def test_waterfall_max_tier_getleads_blocks_later(monkeypatch) -> None:
    gl = MagicMock()
    gl.enabled = True
    gl.calls = 0
    gl.hits = 0
    ...
    ark.find_people.return_value = [
        PersonHit(first_name="A", last_name="B", title="CEO", source_tier="ai_ark")
    ]
    ...
    assert out["vendors_enabled"]["ai_ark"] is False
